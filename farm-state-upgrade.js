(()=>{
  ...
  function addStyle(){const s=DOC().createElement('style');s.id='farm-clean-map-style';s.textContent=`
    ...
    #map .map-canvas .map-ui,#map .map-canvas .map-legend{display:none!important}
    #map .map-clean-live{margin:14px 0 0;position:relative;z-index:2}
    .farm-live-panel{background:#fff;border:1px solid #dce5df;border-radius:18px;padding:18px;box-shadow:0 8px 26px rgba(17,38,29,.08);font-family:Cairo,Tahoma,Arial,sans-serif}
  ...
  function state(){try{return JSON.parse(localStorage.getItem('tanida_dashboard_v2')||'{}')}catch(e){return {}}}
